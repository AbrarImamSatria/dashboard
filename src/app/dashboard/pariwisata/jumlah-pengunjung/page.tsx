...
import JudulHalaman from "@/components/pariwisata/jumlah-pengunjung/JudulHalaman";
...
const DashboardPariwisata = () => {
  const [dataPengunjungTahunan, setDataPengunjungTahunan] = useState<PengunjungTahunan[]>([]);
  const [dataDistribusiTahunan, setDataDistribusiTahunan] = useState<DistribusiTahunan[]>([]);
  const [dataDistribusiBulanan, setDataDistribusiBulanan] = useState<DistribusiBulanan[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const formattedDataPengunjungTahunan: PengunjungTahunan[] = Object.keys(data.jumlah_pengunjung_per_tahun_berdasarkan_kategori_objek_wisata).map((tahun) => ({
          tahun,
          ...data.jumlah_pengunjung_per_tahun_berdasarkan_kategori_objek_wisata[tahun],
          name: tahun,
        }));
        setDataPengunjungTahunan(formattedDataPengunjungTahunan);

        // Format data untuk distribusi pengunjung per tahun
        const formattedDataDistribusiTahunan: DistribusiTahunan[] = Object.keys(data.distribusi_pengunjung_per_tahun_berdasarkan_asal).map((tahun) => ({
          tahun,
        ...
        const formattedDataDistribusiBulanan: DistribusiBulanan[] = Object.keys(data.distribusi_pengunjung_per_bulan_berdasarkan_asal["2023"]).map((bulan) => ({
          bulan,
          ...data.distribusi_pengunjung_per_bulan_berdasarkan_asal["2023"][bulan],
          name: bulan,
        }));
        setDataDistribusiBulanan(formattedDataDistribusiBulanan);

        // Format data untuk objek wisata
        const formattedDataObjekWisata: ObjekWisata[] = data.jumlah_pengunjung_objek_wisata_bantul.map((wisata: any) => ({
          nama: wisata.nama,
        ...
        const maxVisitors = Math.max(...formattedDataObjekWisata.map(item => item.pengunjung));
  ...
  return (
    <div className="flex flex-col gap-8 p-4 max-w-6xl mx-auto">
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <XAxis dataKey="name" />
                <YAxis tickFormatter={formatSumbuYDistribusi} domain={[0, 10]} />
                <Tooltip
      ...
      <div className="border border-gray-200 rounded-lg p-4 bg-white shadow-md">
        ...
        <div className="h-120">
          <ResponsiveContainer width="100%" height={Math.max(dataObjekWisata.length * 50, 500)}>
            <BarChart
              layout="vertical"
